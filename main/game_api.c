...
static int path_urlencode_add(char **dest, const char *path) {
	...
	char *str_urlenc = malloc(3 * len + 1);
	char *aux = NULL;
	int result = -1;
	int aux_len;
...
static int key_val_urlencode_add(char **dest, const char *key, const char *value)
{
	...
	const int aux_len = asprintf(&aux, "%s&%s=%s", *dest ? *dest : "",
			key_urlenc, val_urlenc);
...
bool ga_key_value_add(const char *key, const char *value)
{
	...
	if (len >= 0) {
		ga.keyval_len = len;
		LOGD("%s, %d bytes", ga.keyval, ga.keyval_len);
	} else {
		LOGE("failed");
		err = true;
	}
...
{
	esp_sha1_t sha1_handle;
	uint8_t sha1[SHA1_BYTES] = {0};
	char sha1_str[SHA1_DIGITS + 1] = {0};
	char *aux = NULL;
	int aux_len;

	esp_sha1_init(&sha1_handle);
	esp_sha1_update(&sha1_handle, *url, url_len);
	esp_sha1_update(&sha1_handle, ga.priv_key, ga.priv_key_len);
	esp_sha1_finish(&sha1_handle, sha1);
		...
		uint8_t num_kv_pairs, const char *data, int32_t *body_len)
{
	...
	http_url_set(url);
	http_method_set(method);
	http_open(0);
	http_finish(&status, body_len);
	LOGD("finished with code=%d, length=%d", status, *body_len);
